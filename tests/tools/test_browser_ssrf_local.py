...
All browser backends enforce SSRF protection by default. Users can opt out via
``browser.allow_private_urls: true`` when they explicitly need private-address
navigation.
"""
...
class TestPreNavigationSsrf:
    ...
    # -- Local mode: SSRF still active -----------------------------------------

    def test_local_blocks_private_url_by_default(self, monkeypatch, _common_patches):
        """Local backends must also block private URLs by default."""
...
class TestPostRedirectSsrf:
    ...
    # -- Local mode: redirect SSRF still active --------------------------------
